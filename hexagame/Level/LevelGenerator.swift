...
class LevelGenerator {
    ...
    static func generateLevelHexagons(description: LevelDescription) -> HexagonLevel {
        ...
        for _ in 0..<description.startHexagons {
            availableHexagonIndecies.append(
                HexagonIndex(
                    row: rng.nextInt(upperBound: description.size.height),
                    col: rng.nextInt(upperBound: description.size.width)
                )
            )
        }
        ...
        while hexagons.count < description.totalHexagons {
            let newHexagonIndex = availableHexagonIndecies.remove(at:
                self.rng.nextInt(upperBound: availableHexagonIndecies.count)
            )

            // invalid index
            if newHexagonIndex.col > description.size.width || newHexagonIndex.row > description.size.height ||
                newHexagonIndex.row < 0 || newHexagonIndex.col < 0 {
                continue
            }
            // already exists
        ...
        return HexagonLevel(
            hexagons: simplifiedHexagons,
            gridSize: (cols: maxLeft - minLeft + 1, rows: maxTop - minTop + 1)
        )
    }
    ...
    static func scrambleLevel(level: HexagonLevel) {
        ...
        for hexagon in level.hexagons.values.sorted(
            by: {(hexagon1, hexagon2) in hexagon1.gridIndex < hexagon2.gridIndex}
            ) {
